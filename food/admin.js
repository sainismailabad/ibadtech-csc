...
viewLocation: function(lat, lng) {
    ...
    // ====== RATINGS MODERATION (Admin) ======
    loadRatingsModeration: async function() {
        // Load initial (pending) for both types
        await Promise.all([
            this.loadRestaurantRatingsTable('pending'),
            this.loadProductRatingsTable('pending')
        ]);
    },
        ...
        try {
            ...
            if (snap.exists()) {
                snap.forEach(targetSnap => {
                    const targetId = targetSnap.key;
                    targetSnap.forEach(rSnap => {
                        const r = rSnap.val() || {};
                        const st = this._ratingStatus(r.status);
                        ...
                        rows.push({
                            ...
                            status: st,
                            path: `restaurantRatings/${targetId}/${rSnap.key}`
                        });
                    });
                });
            }
            ...
            if (!rows.length) {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-muted">No ratings</td></tr>';
                return;
            }
            for (const row of rows) {
                ...
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td>${this._safe(row.userName)}</td>
                    <td><span class="badge bg-warning text-dark">${this._safe(row.stars)}</span></td>
                    <td><small>${this._safe(row.comment)}</small></td>
                    <td><small>${this._safe(d)}</small></td>
                    <td>${row.status === 'hidden' ? '<span class="badge bg-danger">hidden</span>' : row.status === 'pending' ? '<span class="badge bg-warning text-dark">pending</span>' : '<span class="badge bg-success">approved</span>'}</td>
                    <td class="text-center">
                        <button class="btn btn-sm btn-success me-1" onclick="Admin.approveRestaurantRating('${row.path}')"><i class="fas fa-check"></i></button>
                        <button class="btn btn-sm btn-outline-danger" onclick="Admin.hideRestaurantRating('${row.path}')"><i class="fas fa-eye-slash"></i></button>
                    </td>
                `;
                tbody.appendChild(tr);
            }
        } catch (e) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-danger">Error: ${this._safe(e?.message || e)}</td></tr>`;
        }
    },
        ...
        try {
                ...
                snap.forEach(targetSnap => {
                    ...
                    targetSnap.forEach(rSnap => {
                        ...
                        rows.push({
                            ...
                            createdAt: r.createdAt || null,
                            status: st,
                            path: `productRatings/${targetId}/${rSnap.key}`
                        });
                    });
                });
            }
            ...
            for (const row of rows) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${row.status === 'hidden' ? '<span class="badge bg-danger">hidden</span>' : row.status === 'pending' ? '<span class="badge bg-warning text-dark">pending</span>' : '<span class="badge bg-success">approved</span>'}</td>
                    <td class="text-center">
                        <button class="btn btn-sm btn-success me-1" onclick="Admin.approveProductRating('${row.path}')"><i class="fas fa-check"></i></button>
                        <button class="btn btn-sm btn-outline-danger" onclick="Admin.hideProductRating('${row.path}')"><i class="fas fa-eye-slash"></i></button>
                    </td>
                `;
                tbody.appendChild(tr);
            }
    ...
    approveRestaurantRating: async function(path) {
        ...
        try {
            await db.ref(path).update({ status: 'approved', moderatedAt: firebase.database.ServerValue.TIMESTAMP });
            const f = document.getElementById('filter-restaurant-rating-status');
            this.loadRestaurantRatingsTable(f ? f.value : '');
    ...
    hideRestaurantRating: async function(path) {
        ...
        try {
            await db.ref(path).update({ status: 'hidden', moderatedAt: firebase.database.ServerValue.TIMESTAMP });
            const f = document.getElementById('filter-restaurant-rating-status');
            this.loadRestaurantRatingsTable(f ? f.value : '');
        } catch (e) { auth.showToast('Error: ' + (e?.message || e)); }
    },
    ...
    approveProductRating: async function(path) {
        ...
        try {
            await db.ref(path).update({ status: 'approved', moderatedAt: firebase.database.ServerValue.TIMESTAMP });
            const f = document.getElementById('filter-product-rating-status');
            this.loadProductRatingsTable(f ? f.value : '');
        } catch (e) { auth.showToast('Error: ' + (e?.message || e)); }
    },
    ...
    // ====== DELIVERY BOY RATINGS (Admin) ======
    loadDeliveryBoyRatings: async function() {
        const tbody = document.getElementById('delivery-boy-ratings-table');
        if (!tbody) return;
        tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4"><div class="spinner-border text-primary"></div></td></tr>';
        try {
            const [ratingsSnap, partnersSnap] = await Promise.all([
                db.ref('deliveryBoyRatings').once('value'),
                db.ref('deliveryPartners').once('value')
            ]);
            tbody.innerHTML = '';
            const rows = [];
                ...
                ratingsSnap.forEach(partnerSnap => {
                    ...
                    partnerSnap.forEach(rSnap => {
                        ...
                        rows.push({
                            ...
                            comment: r.comment || '',
                            createdAt: r.createdAt || null,
                            path: `deliveryBoyRatings/${partnerId}/${rSnap.key}`
                        });
                    });
                });
            }
            rows.sort((a,b) => (b.createdAt || 0) - (a.createdAt || 0));
            if (!rows.length) {
                tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-muted">No delivery boy ratings yet</td></tr>';
                return;
            }
            for (const row of rows) {
                const d = row.createdAt ? this._dateText(row.createdAt) : 'N/A';
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td><strong>${this._safe(row.partnerName)}</strong></td>
                    <td><span class="badge bg-warning text-dark">${this._safe(row.stars)} ⭐</span></td>
                    <td><small>${this._safe(row.comment)}</small></td>
                    <td><small>${this._safe(d)}</small></td>
                    <td class="text-center">
                        <button class="btn btn-sm btn-danger" onclick="Admin.deleteDeliveryBoyRating('${row.path}')"><i class="fas fa-trash"></i></button>
                    </td>
                `;
                tbody.appendChild(tr);
            }
        } catch (e) {
            tbody.innerHTML = `<tr><td colspan="5" class="text-danger">Error: ${this._safe(e?.message || e)}</td></tr>`;
        }
    },
    ...
    deleteDeliveryBoyRating: async function(path) {
        ...
        try {
            ...
            this.loadDeliveryBoyRatings();
        } catch (e) { auth.showToast('Error: ' + (e?.message || e)); }
    },

    filterRestaurantRatings: async function() {
        const sel = document.getElementById('filter-restaurant-rating-status');
        await this.loadRestaurantRatingsTable(sel ? sel.value : '');
    },

    filterProductRatings: async function() {
        const sel = document.getElementById('filter-product-rating-status');
        await this.loadProductRatingsTable(sel ? sel.value : '');
    }
};
