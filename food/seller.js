...
const Seller = {
  ...
  loadProducts: async function () {
    ...
    try {
        ...
        snapshot.forEach(child => {
          ...
          if (p?.sellerId === this.currentUser.uid) {
            ...
            tr.innerHTML = `
              ...
              <td>${p.preparationTime ? p.preparationTime + ' min' : '-'}</td>
      ...
      if (tbody.children.length === 0) tbody.innerHTML = '<tr><td colspan="7" class="text-center py-4 text-muted">No products found</td></tr>';
