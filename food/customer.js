...
window.Foodie = {
    ...
    placeOrder: async function() {
        ...
        // ===== Ismailabad service-area gate =====
        // If restaurant service area polygon is not configured in DB,
        // fall back to the existing radius check (MAX_DELIVERY_RADIUS_KM).
        // This prevents fake orders outside the supported region.
        if (typeof lat !== 'number' || typeof lng !== 'number') {
            alert('Invalid delivery location. Please try again.');
            return;
        }

        const distToCenterKm = calculateDistance(Number(lat), Number(lng), ISMAILABAD_CENTER.lat, ISMAILABAD_CENTER.lng);
        if (distToCenterKm > MAX_DELIVERY_RADIUS_KM) {
            if (geoStatus) {
                geoStatus.innerHTML = `<span class="text-danger"><i class="fas fa-ban me-1"></i>Out of service area</span> (${distToCenterKm.toFixed(1)} km)`;
            }
            if (hintEl) {
                hintEl.className = 'alert alert-danger py-2 mb-2';
                hintEl.innerHTML = 'Sorry! Delivery is available only within Ismailabad service area.';
            }
            alert('Sorry! Out of service area. Delivery is available only within Ismailabad service area.');
            return;
        }
