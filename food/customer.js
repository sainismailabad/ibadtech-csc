...
window.Foodie = {
    ...
    showAutocomplete: function(query) {
        ...
        items.forEach(item => {
            ...
            html += `
                <div onclick="Foodie.goToProduct('${item.id}')" style="display:flex;align-items:center;gap:12px;padding:10px 16px;cursor:pointer;border-bottom:1px solid #f5f5f5;transition:background 0.15s;" onmouseover="this.style.background='#f8f9fa'" onmouseout="this.style.background=''">
    ...
    placeOrder: async function() {
        ...
        try {
            ...
            for (const rid of restaurantIds) {
                ...
                try {
                    const customerEmail = document.getElementById('checkout-email').value.trim() || (this.currentUser && this.currentUser.email ? this.currentUser.email : '');
                    if (customerEmail) {
                        const emailParams = {
                            order_id: orderRef.key.slice(-6).toUpperCase(),
                            email: customerEmail,
                            orders: groupItems.map(item => ({
                                image_url: (item.images && item.images.length > 0) ? item.images[0] : (item.image || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=200'),
                                name: item.name,
                                units: item.qty,
                                price: item.price
                            })),
                            cost: {
                                shipping: groupDeliveryFee,
                                tax: 0,
                                total: groupFinal
                            }
                        };
                        ...
                        fetch('https://api.emailjs.com/api/v1.0/email/send', {
                            ...
                            body: JSON.stringify({
                                service_id: 'service_pjls8b6',
                                template_id: 'template_qcn6ohq',
                                user_id: 'QRaJoZctuaPFHGD54',
                                template_params: emailParams
                            })
                        }).catch(err => console.error('EmailJS request error:', err));
                    }
                } catch (e) {
                    console.error('EmailJS preparation error:', e);
                }
            }
