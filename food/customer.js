...
window.Foodie = {
    ...
    loadProductsNearbyIndex: async function() {
        ...
        try {
            ...
            const products = [];
            if (prodSnap.exists()) {
                prodSnap.forEach(c => products.push({ id: c.key, ...c.val() }));
            }
            ...
            Object.keys(grouped).forEach(cat => {
                                    ...
                                    <div class="product-item-card">
                                        ...
                                        <div class="item-body">
                                            ...
                                            <div class="mt-2">
                                                ...
                                                ` : `
                                                    <button class="add-btn" onclick="event.stopPropagation(); Foodie.updateCart('${p.id}', 1); Foodie.loadProductsNearbyIndex()" ${stock <= 0 ? 'disabled' : ''}>${stock <= 0 ? 'Sold Out' : '+ Add'}</button>
                                                `}
                                            </div>
                                        </div>
                                    </div>
                ...
                flatHtml += items.map(p => {
                    ...
                    const metaArr = [p.category, p.portionSize ? 'Portion: '+p.portionSize : '', p.preparationTime ? p.preparationTime+' min' : '', stock > 0 ? stock+' available' : 'Out of stock'].filter(Boolean);
                    return `
                                    ...
                                    <div class="col-8 col-sm-9">
                                        <div class="p-3">
                                            ...
                                            <div class="mt-3">
                                                ...
                                                ` : `
                                                    <button class="add-btn" onclick="Foodie.updateCart('${p.id}', 1)" ${stock <= 0 ? 'disabled' : ''}>${stock <= 0 ? 'Sold Out' : '+ Add'}</button>
                                                `}
                                            </div>
                                        </div>
                                    </div>
            ...
            container.innerHTML = restaurants.map(r => {
                ...
                return `
                    <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
                        <div class="restaurant-card" data-id="${r.id}" style="cursor:pointer;">
                            <div class="banner-wrap">
                                <img src="${this._safe(banner)}" alt="${this._safe(r.name)}" onerror="this.src='https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=600'">
                                ${offer}
                                <div class="logo-wrap"><img src="${this._safe(logo)}" alt="${this._safe(r.name)}" onerror="this.src='https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=150'"></div>
                            </div>
                            <div class="card-body" onclick="Foodie.openRestaurantDetailModal('${r.id}')">
                                <h5>${this._safe(r.name)}</h5>
                                <p class="text-muted small mb-2">${this._safe(r.cuisine || 'Multi-Cuisine')}</p>
                                ...
                                <p class="text-muted small mt-2 mb-0">${this._safe(r.address || '')}</p>
    ...
    proceedToCheckoutFromCart: function() {
